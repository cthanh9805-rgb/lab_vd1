...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ===== PAGINATION ===== */
        .pagination { display: flex; list-style: none; gap: 4px; padding: 0; margin: 0; align-items: center; }
        .page-item .page-link {
            display: flex; align-items: center; justify-content: center;
        ...
        .page-item.active .page-link {
            background: linear-gradient(135deg, var(--accent), var(--accent-dark));
            border-color: var(--accent); color: #fff; box-shadow: 0 2px 8px rgba(56,189,248,0.3);
        }
        .page-item.disabled .page-link { opacity: 0.4; cursor: not-allowed; }
        .page-item:not(.active):not(.disabled) .page-link:hover {
            border-color: var(--accent); color: var(--accent-light); background: rgba(56,189,248,0.1);
        }
    </style>
    @yield('styles')
</head>
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('products.index') }}"
           class="nav-item {{ request()->routeIs('products.index', 'products.show', 'products.create', 'products.edit') ? 'active' : '' }}">
        ...
        <a href="{{ route('products.trash') }}"
           class="nav-item {{ request()->routeIs('products.trash') ? 'active' : '' }}">
            <div class="nav-icon"><i class="fas fa-trash-alt"></i></div>
            <span>Thùng rác</span>
        </a>
        <a href="{{ route('products.export') }}"
           class="nav-item">
            <div class="nav-icon"><i class="fas fa-file-csv"></i></div>
        ...
        <div class="topbar-right">
            @auth
                <div style="display:flex; align-items:center; gap:10px;">
                    <a href="{{ route('profile.show') }}" style="display:flex; align-items:center; gap:8px; text-decoration:none;" title="Xem hồ sơ cá nhân">
                        <img src="{{ auth()->user()->avatar_url }}" style="width:32px; height:32px; border-radius:50%; object-fit:cover; border:1px solid var(--border);">
                        <span style="font-size:13px; font-weight:600; color:var(--text-primary);">{{ auth()->user()->name }}</span>
                    </a>
                    <form action="{{ route('logout') }}" method="POST" style="display:inline;">
                        @csrf
                        <button type="submit" class="btn btn-outline" style="padding:4px 10px; font-size:12px; color:var(--danger); border-color:rgba(239,68,68,0.3);" title="Đăng xuất">
                            <i class="fas fa-sign-out-alt"></i>
                        </button>
                    </form>
                </div>
            @else
                <a href="{{ route('login') }}" class="btn btn-primary" style="padding:6px 14px; font-size:12px;">
