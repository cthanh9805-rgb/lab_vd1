...
<div style="max-width:1200px; margin:0 auto;">
    ...
    @php
        ...
        $existingMatrix = old('variant_matrix', $product->variants_matrix ?? []);
    @endphp
    ...
    <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card" style="margin-bottom:20px;">
                    ...
                    <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <select name="classification" class="form-control">
                                    ...
                                    @foreach (['Hàng mới'=>'✨ Hàng Mới','Bán chạy'=>'🔥 Bán Chạy','Nổi bật'=>'⭐️ Nổi Bật','Cao cấp'=>'👑 Cao Cấp','Khuyến mãi'=>'🏷️ Khuyến Mãi'] as $v => $l)
                                        <option value="{{ $v }}" {{ old('classification', $product->classification) == $v ? 'selected' : '' }}>{{ $l }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="grid-2" style="gap:12px;">
                            <div class="form-group" style="margin-bottom:0;">
                                ...
                                <div class="input-group">
                                    <input type="number" name="original_price" class="form-control" value="{{ old('original_price', $product->original_price) }}" min="0">
                        ...
                        <div style="margin-top:8px;">
                            <label class="form-label">Tổng tồn kho (tự tính)</label>
                            <input type="number" name="stock" id="totalStockInput" class="form-control" value="{{ old('stock', $product->stock) }}" min="0" readonly style="background:rgba(255,255,255,0.05); color:var(--accent-light); font-weight:700;">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:12px;">
                            <div class="form-group" style="margin-bottom:0;">
                                ...
                                <input type="number" name="heel_height" class="form-control" value="{{ old('heel_height', $product->heel_height) }}" min="0" max="20">
                            </div>
                            <div class="form-group" style="margin-bottom:0;">
                                <label class="form-label">🧵 Chất liệu</label>
                                <select name="material" class="form-control">
                                    <option value="">-- Chọn --</option>
                                    @foreach (['Da thật','Da PU','Da PU cao cấp','Da bóng','Da mịn','Da thật mềm','Nhung','Vải canvas','Vải canvas + Cói','PVC trong suốt','Vải canvas + Cói thêu'] as $m)
                                        <option value="{{ $m }}" {{ old('material', $product->material) == $m ? 'selected' : '' }}>{{ $m }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group" style="margin-bottom:0;">
                                <label class="form-label">⚖️ Cân nặng (gram)</label>
                                <input type="number" name="weight" class="form-control" value="{{ old('weight', $product->weight) }}" min="0">
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="form-group" style="margin-bottom:0;">
                                ...
                                <select name="origin" class="form-control">
                                    ...
                                    @foreach (['Việt Nam','Hàn Quốc','Nhật Bản','Ý (Italy)','Pháp (France)','Quảng Châu (Trung Quốc)','Mỹ (USA)','Anh (UK)','Đức (Germany)','Thái Lan'] as $c)
                                        <option value="{{ $c }}" {{ old('origin', $product->origin) == $c ? 'selected' : '' }}>🌐 {{ $c }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group" style="margin-bottom:0;">
                                <label class="form-label">Mã giảm giá</label>
                                <input type="text" name="discount_code" class="form-control" value="{{ old('discount_code', $product->discount_code) }}" style="text-transform:uppercase;">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        @if ($product->image)
                            <div style="margin-bottom:12px; border-radius:10px; overflow:hidden; border:1px solid var(--border);">
                                <img src="{{ \Illuminate\Support\Str::startsWith($product->image, 'http') ? $product->image : asset($product->image) }}" alt="{{ $product->name }}" style="width:100%; max-height:180px; object-fit:cover; display:block;">
                            </div>
                        @endif
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label class="form-label">🖼️ Gallery hiện tại (tích xoá ảnh)</label>
                            <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:8px;">
                                @foreach ($product->images as $img)
                                <div style="position:relative; border-radius:8px; overflow:hidden; border:1px solid var(--border);">
                                    <img src="{{ asset($img->image_path) }}" style="width:100%; height:80px; object-fit:cover; display:block;">
                                    <label style="position:absolute; top:4px; right:4px; background:rgba(239,68,68,0.85); width:22px; height:22px; border-radius:50%; display:flex; align-items:center; justify-content:center; cursor:pointer;">
                        ...
                        <div class="form-group">
                            <label class="form-label">🖼️ Thêm ảnh Gallery mới</label>
                            <input type="file" name="gallery[]" multiple accept="image/*" class="form-control" style="padding:8px;">
                        </div>
                        ...
                        <div class="toggle-group">
                            ...
                            <span id="statusLabel" style="font-size:14px; color:var(--text-primary); font-weight:500;">{{ $isActive ? '✅ Đang bán' : '❌ Ngừng bán' }}</span>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="size-chips">
                            ...
                            <label>
                                <input type="checkbox" name="sizes[]" value="{{ $sz }}" class="size-checkbox"
                                       {{ in_array($sz, $selectedSizes) ? 'checked' : '' }} onchange="renderMatrix()">
                        ...
                        <div class="color-swatches">
                            ...
                            <div>
                                <input type="checkbox" name="colors[]" value="{{ $cn }}" class="color-checkbox" id="color_{{ $cn }}"
                                       {{ in_array($cn, $selectedColors) ? 'checked' : '' }} onchange="renderMatrix()">
        ...
        <div style="display:flex; gap:12px; margin-top:24px;">
            <button type="submit" class="btn btn-primary" style="padding:12px 32px; font-size:15px;"><i class="fas fa-save"></i> Cập nhật sản phẩm</button>
...
@section('scripts')
<script>
const existingMatrix = @json($existingMatrix);
...
function renderMatrix() {
    ...
    colors.forEach(cl => {
        ...
        sizes.forEach(sz => {
            const val = (existingMatrix[cl] && existingMatrix[cl][sz] !== undefined) ? existingMatrix[cl][sz] : 0;
            h += `<td style="text-align:center;padding:6px;"><input type="number" name="variant_matrix[${cl}][${sz}]" class="form-control matrix-cell" data-color="${cl}" data-size="${sz}" value="${val}" min="0" oninput="calcTotals()" style="width:65px;text-align:center;padding:4px 6px;font-size:12px;margin:0 auto;"></td>`;
        });
